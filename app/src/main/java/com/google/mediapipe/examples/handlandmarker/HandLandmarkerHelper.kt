...
class HandLandmarkerHelper(
    ...
    private fun setupInterpreter() {
        try {
            val modelBuffer = context.assets.open("asl_model_quantized.tflite").use {
            ...
            val msg = "ASL_DEBUG: Model Loaded! Shape: ${inputShape?.contentToString()}"
            Log.e("ASL_DEBUG", msg)
            
            // Show a toast so we know for sure it initialized
            (context as? android.app.Activity)?.runOnUiThread {
                Toast.makeText(context, msg, Toast.LENGTH_LONG).show()
            }
        } catch (e: Exception) {
            Log.e("ASL_DEBUG", "Failed: ${e.message}")
        }
    }

    fun classifySign(result: HandLandmarkerResult): String? {
        ...
        // Match Python logic: Use only X and Y relative to wrist
        // We flip the X coordinate (multiply by -1) to "un-mirror" the front camera for the AI
        val input = FloatArray(landmarks.size * 2)
        
        for (i in landmarks.indices) {
            input[i * 2] = -(landmarks[i].x() - wrist.x())
            input[i * 2 + 1] = landmarks[i].y() - wrist.y()
        }
        ...
        // Detailed log to see what the model is thinking
        val top3Indices = probabilities.indices.sortedByDescending { probabilities[it] }.take(3)
        val debugInfo = top3Indices.joinToString { i -> "${labels[i]}: ${String.format("%.2f", probabilities[i])}" }
        Log.d(TAG, "Top Predictions: $debugInfo")

        val maxIndex = probabilities.indices.maxByOrNull { probabilities[it] } ?: -1
        
        // 0.4 threshold to match responsiveness
        return if (maxIndex != -1 && probabilities[maxIndex] > 0.4) {
            labels[maxIndex]
        } else {
            null
        }
    ...
    fun detectImage(image: Bitmap): ResultBundle? {
        ...
        handLandmarker?.detect(mpImage)?.also { landmarkResult ->
            ...
            val signLabel = classifySign(landmarkResult)
            return ResultBundle(
    ...
    private fun returnLivestreamResult(
        ...
        val signLabel = classifySign(result)
